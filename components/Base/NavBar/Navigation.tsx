...
export default function Navigation ({isOpen,toggleOpen}:{isOpen:boolean,toggleOpen:Cycle}) {
 ...
 return   <motion.ul initial={false} variants={variants} animate={isOpen ? "open" : "closed"} className={'absolute top-[70px] flex flex-col  gap-8'}>
        {Items.map((item,index) => (
            <ListItem  key={index} text={item.text} fn={item.fn}/>
        ))}
