    ...
    return <div className={'flex flex-col gap-3'}>
        <div className={'flex justify-between gap-4 thin-round-black-border px-2.5 py-4'}>
            ...
            <div className={'flex flex-col gap-2 ml-auto'}>
                {isMine || blogOwner ? <span className={'cursor-pointer thin-round-black-border text-center p-1 w-16 h-8'} onClick={editCommentClick}>수정</span> : null}
                {isMine || blogOwner ? <span className={'cursor-pointer thin-round-black-border text-center p-1 w-16 h-8 '} onClick={deleteCommentClick}>{deleteCommentMutation.isLoading ? <LoadingSpinner />:"삭제"}</span> : null}
            </div>
        </div>
        {editEditor ? (<form className={'flex gap-4 w-full'} onSubmit={editCommentSubmit}>
            <textarea className={'w-full thin-round-black-border resize-none p-2 focus:outline-none'} ref={commentRef}/>
            <button className={'h-full w-16 thin-round-black-border m-auto text-center hover:ring-2 hover:ring-offset-2 hover:ring-black'}>{editCommentMutation.isLoading ? <LoadingSpinner />:"등록"}</button>
        </form>) : null }
