...
const fontByLevel = (level:number):string =>{
    ...
    return (
        <motion.div
            ...
            layout transition={spring} className={cls(isVisible ? "mobile:block" : "",`fixed right-0 top-32 min-w-md hidden bg-white p-4 space-y-4 rounded-sm border-2 border-gray-200 shadow-sm shadow-slate-300`)}>
            ...
            <ul >
                {toc.map((item, index) => (
                    <motion.li whileHover={{scale: 1.1}}
                        ...
                        <a className={`${fontByLevel(item.level)}`} onClick={(e) => {
